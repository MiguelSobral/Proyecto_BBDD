...
def correlacion(ratings_u, media_u, ratings_v, media_v):
    ...
    if suma_1 == 0 or suma_2 == 0:
        print("El denominador de la correlacion es 0")
        return
...
def usuario_mas_vecinos():
    ...
    if resultado is None:
        ...
        print("Usuario con mas vecinos es", resultado["reviewerID"], "con", resultado["vecinos"])
...
def obtener_articulos_aleatorios():
    ...
    with conexion:
        ...
        while not cantidad_posible:
            ...
            datos = cursor.fetchall()  
            datos_available = len(datos)

            if datos_available < num_articulos:
                print("No hay tantos datos, hay menos de", num_articulos, "en", categoria, "hay", datos_available)
            else:
                cantidad_posible = True
        
        list_asins = [dato[0] for dato in datos]

    return list_asins, mapeo_categoria
...
def cargar_reviews_por_articulo_neo4J():
    ...
    reviews_asin = reviews_por_articulo()
            ...
            for reviewerID, (reviewTime, overall) in reviews.items():
                query = """
                    ...
                    SET r.reviewTime = $reviewTime,
                        r.overall = $overall
                """
                session.run(query, asin=asin, reviewerID=reviewerID, reviewTime=str(reviewTime), overall=float(overall))
...
def opcion_2():
    ...
    print("Se ha terminado la carga de datos")
...
def primeros_usuarios():
    ...
    usuarios = [dato[0] for dato in datos]
...
def obtener_reviews_primeros_usuarios():
    ...
    for reviewerID, tipo_producto, num_productos in datos:
        if reviewerID not in reviews:
            reviews[reviewerID] = {}
        reviews[reviewerID][tipo_producto] = num_productos
            ...
            if len(tipo_cantidad) >= 2:
                for tipo_producto, cantidad in tipo_cantidad.items():
                    query = """
                        ...
                        MERGE (u)-[r:REVIEWS]->(t)
                        SET r.num_reviews = $cantidad
                    """
                    session.run(query, reviewerID=reviewerID, tipo_producto=tipo_producto, cantidad=cantidad)
...
def obtener_usuarios_articulos():
    ...
    condicion_valida = " OR ".join(["(asin = %s AND tipo_producto = %s)"] * len(articulos))
    ...
    for asin, tipo_producto, reviewerID in datos:
        ...
        try:
            reviews[key].append(reviewerID)
        except KeyError:
            reviews[key] = [reviewerID]
...
def articulos_en_comun():
    ...
    for lista_users in reviews.values():
        for reviewerID in lista_users:
            if reviewerID not in users:
                users.append(reviewerID)
    ...
    for reviewerID, asin, tipo_producto in datos:
        ...
        try:
            ...
            articulos_por_user[reviewerID] = {articulo}

    comunes = {}
    usuarios = [key for key in articulos_por_user.keys()]
...
def cargar_top_articulos_usuarios_neo4J():
    ...
    with driver.session() as session:
        for (asin, tipo_producto), usuarios in reviews.items():
            for reviewerID in usuarios:
                query = """
                    ...
                    MERGE (u)-[:REVIEWS]->(a)
                """
                session.run(query, reviewerID=reviewerID, asin=asin, tipo_producto=tipo_producto)
        ...
        for (user_u, user_v), num_comun in comunes.items():
            ...
            """
            session.run(query, user_u=user_u, user_v=user_v, cantidad=num_comun)
